...
float SmoothStepFunction::getValue(sf::Int32 time) const {
...
float SmoothStepFunction::smoothstep(float edge0, float edge1, float x) const {
    x = clamp((x - edge0) / (edge1 - edge0), edge0, edge1);
    return x * x * (3 - 2 * x);
}

float SmoothStepFunction::clamp(float x, float lowerLimit, float upperLimit) const {
    if (x < lowerLimit) {
        x = lowerLimit;
    }
    if (x > upperLimit) {

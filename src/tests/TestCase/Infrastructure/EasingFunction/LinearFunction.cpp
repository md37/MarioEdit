...
#include "classes/Infrastructure/Animation/EasingFunction/LinearFunction.hpp"
...
LinearFunction createLinearReversedFunction() {
    LinearFunction function(1500, 2.0f, 1.0f);
    return function;
}

TEST(EasingFunction_LinearFunction_Reversed_Test, test_0percent_value) {
    auto function = createLinearReversedFunction();

    ASSERT_FLOAT_EQ(2.0f, function.getValue(0));
}

TEST(EasingFunction_LinearFunction_Reversed_Test, test_25percent_value) {
    auto function = createLinearReversedFunction();

    ASSERT_FLOAT_EQ(1.75f, function.getValue(375));
}

TEST(EasingFunction_LinearFunction_Reversed_Test, test_50percent_value) {
    auto function = createLinearReversedFunction();

    ASSERT_FLOAT_EQ(1.5f, function.getValue(750));
}

TEST(EasingFunction_LinearFunction_Reversed_Test, test_75percent_value) {
    auto function = createLinearReversedFunction();

    ASSERT_FLOAT_EQ(1.25f, function.getValue(1125));
}

TEST(EasingFunction_LinearFunction_Reversed_Test, test_100percent_value) {
    auto function = createLinearReversedFunction();
